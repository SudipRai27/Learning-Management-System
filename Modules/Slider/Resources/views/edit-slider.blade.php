...
@include('slider::tabs')
<div class="form-header"><p class="form-header">Edit Slider</p></div>
<form action="{{route('edit-slider-post', $slider->id)}}" method="POST" enctype="multipart/form-data">
	<div class="box-body">
		@include('backend.partials.errors')
		<div class="form-header2">Update</div>
		<div class="form-group">
			<label>Title: </label>
				<input type="text" name="title" id ="title" class="form-control" value="{{ $slider->title}}" placeholder="Title" required>
		...
		<div class="row">
			<div class="col-sm-6">
				<div class="form-group">	
		        	...
		        	<input id="sort_order" class="form-control" type="number" placeholder="Sort Order" name="sort_order" value="{{$slider->sort_order}}" min="1" required>
			...
			<div class="col-sm-6">
				<div class="form-group">
					...
					<select class="form-control" name="is_active">
						<option value="yes" @if($slider->is_active == 'yes') selected @endif>Yes</option>
						<option value="no" @if($slider->is_active == 'no') selected @endif>No</option>
					</select>
					@if($errors->first('is_active'))  
					<span class="error-badge" @if($slider->is_active == 'no') selected @endif> {{$errors->first('is_active')}}</span>
					@endif 					
				</div>
			</div>
			<div class="col-sm-6">
				<label>Current Image: </label>
				@if(count($slider->resource))

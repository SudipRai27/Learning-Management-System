...
@endsection
@section('content')
<div class="form-header"><p class="form-header">Manage Resources</p></div>
...
<table class="table table-bordered table-hover" id="lecture-table">   
	...
	<tbody>
		...
		<td>
			<form action="{{route('remove-lecture-resource', $resource['resource_id'])}}" method="POST">
...
<div class="form">
	...
	<form action="{{route('upload-lecture-resources')}}" enctype="multipart/form-data" method="POST">
		<div class="form-group">	                    
		    <label>Multiple Files: (Allowed: pptx,txt,doc,docx,pdf: max 5MB each)</label>            
		    <input id="kv-explorer" type="file" name="files[]" class="@if($errors->first('files')) form-error @endif" multiple>
		    <input type="hidden" name="lecture_id" value="{{$lecture->id}}">
		    <input type="hidden" name="session_id" value="{{$lecture->session_id}}">
